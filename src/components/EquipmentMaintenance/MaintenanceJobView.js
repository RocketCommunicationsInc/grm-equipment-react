                  ...
                  maintenanceJobs.map(function (job) {
                    return (
                      <div className="job-card">
                        ...
                        <h4 className="job-card__progress">{mapJobProgress(job.progressStep)}</h4>
                        <div className="job-card__meta">
                          <label className="job-card__meta__label">Job Type</label>
                          <div className="job-card__meta__value">{mapJobType(job.type)}</div>
                        </div>
                        <div className="job-card__meta">
                          <label className="job-card__meta__label">Year</label>
                          <div className="job-card__meta__value">{formatYear(job.startTime)}</div>
                        </div>
                        <div className="job-card__meta">
                          <label className="job-card__meta__label">DOY</label>
                          <div className="job-card__meta__value">{formatDayOfYear(job.endTime)}</div>
                        </div>
                        <div className="job-card__meta">
                          <label className="job-card__meta__label">Start</label>
                          <div className="job-card__meta__value">{formatReadableTime(job.startTime)}</div>
                        </div>
                        <div className="job-card__meta">
                          <label className="job-card__meta__label">Stop</label>
                          <div className="job-card__meta__value">{formatReadableTime(job.endTime)}</div>
                        </div>
                        <RuxButton className="rux-button" on-click="viewJobDetails">
                          View Details
                        </RuxButton>
                      </div>
                    );
            ...
            <div className="grid-zone__content maintenance-log">
              <header className="maintenance-log-header">
                <div className="maintenance-log__header-labels">
                  ...
                  <div className="maintenance-log__event__technician">Technician</div>
                  <div className="maintenance-log__event__description">Description</div>
                </div>
              </header>
              <ol className="maintenance-log__events">
                {maintenanceLog &&
                  maintenanceLog.map(function (job) {
                    return (
                      <li className="maintenance-log__event">
                        <div className="maintenance-log__event__name">{job.id}</div>
                        <div className="maintenance-log__event__type">{mapJobType(job.type)}</div>
                        <div className="maintenance-log__event__created">
                          {formatDayOfYear(job.createdTime)} {formatReadableTime(job.createdTime)}
                        </div>
                        <div className="maintenance-log__event__start">
                          {formatDayOfYear(job.startTime)} {formatReadableTime(job.startTime)}
                        </div>
                        <div className="maintenance-log__event__stop">
                          {formatDayOfYear(job.endTime)} {formatReadableTime(job.endTime)}
                        </div>
                        <div className="maintenance-log__event__technician">{job.technician}</div>
                        <div className="maintenance-log__event__description">{job.description}</div>
                      </li>
                    );
                  })}
              </ol>
